...
Todos los artefactos de evaluación residen en el directorio \texttt{tests/} del repositorio. Cada cuadro de este capítulo se respalda con un fichero concreto, de modo que los valores reportados son reproducibles y auditables: la suite E2E (Cuadro~\ref{tab:e2e-categorias}) en \texttt{tests/results/testing\_general.md}, el banco de preguntas de la capa de profundidad RAG (Cuadro~\ref{tab:rag-cobertura}) en \texttt{tests/plans/rag\_asignaturas\_manual.md} y su ejecución en \texttt{tests/results/rag\_asignaturas.md}, los resultados de \texttt{rasa test core} (Cuadro~\ref{tab:nlu-resultados}) en \texttt{tests/results/stories.md}, las métricas no funcionales de latencia y coste (Cuadros~\ref{tab:latencia} y~\ref{tab:coste-extrapolado}) en \texttt{tests/results/coste\_latencia.md} y la suite automatizada del panel (Cuadro~\ref{tab:admin-resultados}) en \texttt{tests/test\_admin.py}. La auditoría del tráfico real (Cuadro~\ref{tab:trafico-resultados}) se realiza sobre la tabla \texttt{conversation\_log} directamente desde el panel de administración, sin fichero intermedio.
...
Los identificadores de los casos de prueba siguen la convención \texttt{<tipo>-<origen><número>}: la letra de tipo indica la naturaleza del veredicto (\texttt{E} para error, \texttt{C} para confusión entre flujos, \texttt{X} para sesión excluida) y el sufijo alfanumérico referencia el fichero de resultados correspondiente en \texttt{tests/results/}. Las decisiones de diseño citadas como \textbf{D-\textit{nnn}} se documentan en el repositorio bajo \texttt{docs/sprints/}.
...
Brecha funcional no prevista & X-P06 & Nuevo flujo \texttt{consulta\_\allowbreak titulacion\_\allowbreak por\_\allowbreak tema} en fase 2 \\ \hline
\end{tabular}
\end{table}
...
El coste se calcula aplicando las tarifas públicas de Gemini Flash Lite (0,075~USD por millón de \textit{tokens} de entrada y 0,30~USD por millón de \textit{tokens} de salida) y un tipo de cambio de 0,92~EUR/USD. Se elige Flash Lite por ser el modelo de facturación activa más económico del catálogo de Google~\cite{geminiPricing} en la fecha de redacción, lo que proporciona la cota inferior del coste hipotético: cualquier alternativa de pago con mayor capacidad (Gemini 2.5 Flash, recomendado como paso intermedio en la Sección~\ref{sec:costes}) resultaría más cara. Sobre 216 llamadas medidas, el coste medio por turno se sitúa en \textbf{0,012~céntimos de euro}, con un coste mediano de 0,008 céntimos y un percentil 95 de 0,036 céntimos. El turno más caro de la suite alcanzó 0,064 céntimos.

Para extrapolar este coste al despliegue completo del prototipo en la ETSII (con una población estimada de 700 alumnos del Grado en Ingeniería Informática) se han considerado tres escenarios de intensidad de uso, recogidos en la Tabla~\ref{tab:coste-extrapolado}. Los tres valores de turnos por alumno y mes se anclan en evidencia distinta para acotar el rango económico, en lugar de fijarse de forma arbitraria. El escenario \textbf{conservador} (8 turnos/alumno/mes) se deriva del piloto. En los 25 días de captura se registraron 59 sesiones y 203 turnos auditados (Sección~\ref{subsec:trafico-resultados}); bajo el supuesto de que un mismo usuario entró entre una y tres veces como máximo, el número de usuarios distintos se sitúa entre $\sim$20 y $\sim$59, lo que arroja una tasa observada de 2--4 sesiones por usuario y mes. Multiplicada por la longitud media de sesión ($\sim$3{,}4 turnos por sesión), esta tasa equivale a unos 7--14 turnos por alumno y mes, rango en cuyo extremo bajo se sitúa el valor de 8 utilizado como cota inferior. El escenario \textbf{realista} (20 turnos/alumno/mes) se ancla en la literatura sobre chatbots universitarios desplegados sobre poblaciones completas. Goel y Polepeddi~\cite{goel2018jillwatson} reportan del orden de 10\,000 mensajes por semestre con aproximadamente 300 alumnos en la asistente virtual Jill~Watson del Georgia Tech OMSCS, equivalentes a $\sim$8 mensajes por alumno y mes a lo largo de un cuatrimestre lectivo; asumiendo entre dos y tres turnos por consulta en sistemas conversacionales con seguimiento de \textit{slots} como el aquí evaluado, el orden de magnitud encaja con los $\sim$20 turnos del escenario realista. El rango es además consistente con la adopción observada en los pilotos universitarios de AdmitBot~\cite{gupta2019admitbot} y EDUBOT~\cite{colace2018edubot}. El escenario de \textbf{pico} (50 turnos/alumno/mes) traduce la concentración de consultas durante la semana de exámenes mediante un cálculo explícito: un alumno que consulte el bot a diario durante los $\sim$10 días hábiles previos a la convocatoria, con un promedio de cinco turnos por consulta (horarios, profesorado por grupo, criterios de evaluación, aulas), alcanza la cifra mensual de 50 turnos. La cifra extrapolada corresponde \emph{únicamente} al coste hipotético de las llamadas al modelo de lenguaje en una tarifa de pago equivalente: en el despliegue actual, este coste es nulo porque el sistema utiliza el plan gratuito de Gemma. La cifra mostrada sirve para acotar la magnitud económica del componente LLM frente a alternativas comerciales, no como coste total de operación.
...
\paragraph{Validez de constructo.} La métrica principal del estudio, la \textbf{tasa de éxito} de cada capa, es una aproximación útil pero no exhaustiva al concepto de calidad de un asistente conversacional. No captura dimensiones subjetivas como la satisfacción del usuario ni la utilidad percibida, ambas evaluables con escalas estandarizadas como SUS~\cite{brooke1996sus} o CSAT que se reservan para una segunda iteración. La complementariedad de las cinco capas (E2E con datos sintéticos, RAG sobre planes reales, NLU sobre \textit{stories}, tráfico real y suite de admin) mitiga parcialmente esta limitación al cubrir aspectos distintos del mismo constructo, pero la equivalencia entre las cinco no es plena.
