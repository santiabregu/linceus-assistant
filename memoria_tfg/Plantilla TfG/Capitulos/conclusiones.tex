...
\paragraph{Calibración empírica documentada en lugar de umbrales por convención.} Los porcentajes de éxito de los cuatro sistemas comparables se publican sin una descripción del procedimiento por el que se fijaron los umbrales internos del sistema (similitud semántica, fuzzy matching, fallback NLU). En Linceus, la calibración de los tres umbrales críticos se documenta de manera trazable: 120 casos de prueba en tres iteraciones para el matching de profesorado (Sección~\ref{sec:impl-shared}), elevación del umbral de cambio de titulación a 0,85 tras observar cambios silenciosos durante la revisión R3 (Sección~\ref{sec:impl-contexto}) y configuración del par \texttt{threshold = 0,8} / \texttt{ambiguity\_threshold = 0,10} del clasificador de fallback (Sección~\ref{subsec:dd-pipeline}). Esta trazabilidad convierte cada cifra de la suite de pruebas en una decisión reproducible.
