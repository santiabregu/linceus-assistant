...
Pylint se configura en \texttt{.pylintrc} con una longitud máxima de línea de 120 caracteres y supresión de los errores de importación propios del entorno Rasa (paquetes como \texttt{rasa\_sdk} o \texttt{rapidfuzz} residen en el entorno virtualizado y no son accesibles desde el intérprete de análisis estático). Se ejecuta sobre \texttt{actions/} y \texttt{admin/} con el flag \texttt{--errors-only} para detectar errores reales antes de integrar cambios en la rama principal.

Flake8 amplía la cobertura de estilo con verificaciones que pylint no contempla: espacios en blanco sobrantes, imports no utilizados, f-strings sin marcadores de sustitución, longitud de línea e indentación de continuación. Se ejecuta con \texttt{--max-line-length=120} sobre los directorios \texttt{actions/}, \texttt{admin/} y \texttt{rag/}. Las líneas que superan el límite por contener cadenas de texto largas no partibles (prompts al LLM, URLs de constantes, sentencias SQL) se suprimen con la directiva \texttt{\# noqa: E501}; los imports que son re-exportaciones intencionadas en ficheros \texttt{\_\_init\_\_.py} se suprimen con \texttt{\# noqa: F401}; y las instrucciones dobles en una sola línea que forman un par semántico indivisible (\texttt{conds.append}~/~\texttt{params.append}) se suprimen con \texttt{\# noqa: E702}. Los imports posteriores a \texttt{sys.path.insert} —necesarios para que los scripts del pipeline RAG y el módulo de profesores sean ejecutables de forma autónoma fuera del paquete— se suprimen con \texttt{\# noqa: E402}. El resto del código cumple PEP~8 sin supresiones.
...
El tiempo dedicado al proyecto no se mide con herramientas de \textit{time tracking} formales, sino que se estima a partir de la frecuencia de commits y del contenido de los registros de sprint.
