...
\node[plllm, left=14mm of dec] (clasif) {Gemini clasificador (heurística + LLM)};
...
\node[plllm, right=18mm of dec] (gensql) {Gemini redacta respuesta con datos estructurales};
...
\begin{lstlisting}[language=python, caption={Cascada de resolucion de \texttt{resolver\_asignatura()} (\texttt{actions/asignaturas/actions.py}).}, label={lst:asig-cascada}]
def resolver_asignatura(pregunta, tracker, contexto_titulacion,
                        usar_seguimiento=True):
    # 1. Entidad NLU del ultimo mensaje (extractor de Rasa)
    nombre = extraer_nombre_asignatura(tracker, ...)

    # 2. Expansion de acronimo contra ALIAS_ASIGNATURAS (FP -> Fundamentos...)
    if nombre:
        nombre = _expandir_alias(nombre, contexto_titulacion)

    # 3. Sin entidad NLU: regex sobre alias conocidos en el texto crudo
    if not nombre:
        for alias in sorted(ALIAS_ASIGNATURAS, key=len, reverse=True):
            if re.search(r"\b" + re.escape(alias) + r"\b", pregunta.lower()):
                nombre = _expandir_alias(alias, contexto_titulacion)
                break

    # 4. Seguimiento: heredar slot ultimo_nombre_asignatura si <=3 turnos atras
    if usar_seguimiento and not nombre:
        if _contar_turnos_desde_slot(tracker, "ultimo_nombre_asignatura") <= 3:
            nombre = tracker.get_slot("ultimo_nombre_asignatura")

    # 5. Fuzzy matching contra nombres reales de la BD (rapidfuzz, umbral 85)
    if not nombre:
        nombre = _resolver_nombre_desde_texto(pregunta, contexto_titulacion)

    # SELECT con ILIKE; si no hay resultados, fallback con fuzzy reordering
    # (token_set_ratio contra la pregunta original para desempatar candidatos).
    ...
\end{lstlisting}
...
\begin{lstlisting}[language=python, caption={Atajos a RAG en \texttt{ActionConsultaProfesor} (\texttt{actions/profesores/actions.py}).}, label={lst:prof-atajos}]
...
if codigo_asignatura and _pregunta_menciona_rol_rag(pregunta):
    chunks = _rag_chunks_plan_docente(
        codigo_asignatura, grupo=grupo_detectado,
    )
    if chunks:
        respuesta = _generar_respuesta_rag(pregunta, chunks, nombre_asignatura)
        if respuesta:
            dispatcher.utter_message(text=respuesta)
            return [...]
    # Si el RAG no devuelve respuesta, cae al flujo SQL normal.

# Atajo 2: asignatura + grupo concreto -> RAG con filtro de grupo
...
\begin{itemize}
    ...
    \item \texttt{profesores}: tabla de consulta paginada del directorio PDI. El \textit{endpoint} \texttt{/api/admin/profesores/sync} lanza el scraper de \texttt{us.es} que actualiza la tabla \texttt{profesores} y rellena \texttt{profesor\_asignatura} para el curso vigente (Figura~\ref{fig:impl-admin-profesores}).
