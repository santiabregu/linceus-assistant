...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{img/diagrama_despliegue.png}
...
\textbf{Justificación:}
\begin{itemize}
    \item 800 caracteres ($\approx$150--200 tokens) es suficiente para capturar un párrafo o sección temática completa, en línea con los valores recomendados por LangChain~\cite{langchainChunking}.
...
\begin{enumerate}
    \item \texttt{SpacyNLP} (modelo \texttt{es\_core\_news\_md})~\cite{rasadocs}: carga del modelo de lenguaje preentrenado en español, base de los \textit{embeddings} léxicos densos.
    \item \texttt{SpacyTokenizer}~\cite{rasadocs}: tokenización respetando la segmentación morfológica del modelo de español.
    \item \texttt{RegexFeaturizer}~\cite{rasadocs}: patrones regex para códigos de asignatura y formatos especiales.
    \item \texttt{LexicalSyntacticFeaturizer}~\cite{rasadocs}: características gramaticales del contexto local.
    \item \texttt{CountVectorsFeaturizer} (palabra)~\cite{rasadocs}: n-gramas de palabras (1--2).
    \item \texttt{CountVectorsFeaturizer} (carácter)~\cite{rasadocs}: n-gramas de caracteres (2--4); los n-gramas de un solo carácter se descartaron por aportar ruido sin mejorar la robustez frente a typos.
    \item \texttt{SpacyFeaturizer}~\cite{rasadocs}: \textit{features} densas derivadas del modelo de spaCy, complementarias a las cuentas léxicas.
    ...
    \item \texttt{EntitySynonymMapper}~\cite{rasadocs}: normalización de sinónimos de entidades.
    \item \texttt{ResponseSelector}~\cite{rasadocs}: selección de respuestas tipo FAQ (150 épocas).
    \item \texttt{FallbackClassifier}~\cite{rasadocs}: detección de baja confianza, configurada con \texttt{threshold = 0,8} y \texttt{ambiguity\_threshold = 0,10}.
\end{enumerate}
