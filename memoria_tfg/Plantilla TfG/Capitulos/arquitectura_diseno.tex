...
\textbf{Contexto:} El pipeline NLU de Rasa~\cite{rasadocs,bocklisch2017rasa} debe ser capaz de clasificar correctamente intenciones en español, tolerando errores ortográficos, abreviaturas y variaciones léxicas propias del lenguaje coloquial estudiantil.

\textbf{Decisión:} Se configuró un pipeline de 9 componentes que combina \textbf{featurización a nivel de palabra} (n-gramas 1--2) y \textbf{a nivel de carácter} (n-gramas 1--4), siguiendo el diseño \textit{multi-featurizer} recomendado en la documentación oficial~\cite{rasadocs}:

\begin{enumerate}
    \item \texttt{WhitespaceTokenizer}~\cite{rasadocs}: Tokenización por espacios en blanco.
    ...
    \item \texttt{CountVectorsFeaturizer} (carácter)~\cite{rasadocs}: N-gramas de caracteres (1--4).
    \item \texttt{DIETClassifier}~\cite{bocklisch2020diet}: Clasificador dual de intenciones y entidades (150 épocas, dropout 0.2).
    \item \texttt{EntitySynonymMapper}~\cite{rasadocs}: Normalización de sinónimos de entidades.
    \item \texttt{ResponseSelector}~\cite{rasadocs}: Selección de respuestas tipo FAQ.
    \item \texttt{FallbackClassifier}~\cite{rasadocs}: Detección de baja confianza (umbral 0.7).
\end{enumerate}
...
\begin{itemize}
    ...
    \item El umbral de fallback de 0.7 equilibra la precisión (evitar respuestas incorrectas) con la cobertura (no rechazar demasiadas consultas legítimas), valor coherente con los rangos reportados en \textit{benchmarks} de detección de intenciones~\cite{casanueva2020efficient}.
    \item El \texttt{DIETClassifier} con 150 épocas y dropout de 0.2 proporciona un modelo robusto sin sobreajuste al conjunto de entrenamiento, en línea con la configuración recomendada por sus autores~\cite{bocklisch2020diet}.
\end{itemize}
