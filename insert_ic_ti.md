# Inserción de datos: Titulaciones GII-IC y GII-TI

> Requisitos previos: asegúrate de que existen constraints UNIQUE sobre `titulaciones.codigo` y `asignaturas.codigo`.
>
> ```sql
> ALTER TABLE titulaciones ADD CONSTRAINT IF NOT EXISTS titulaciones_codigo_uk UNIQUE (codigo);
> ALTER TABLE asignaturas  ADD CONSTRAINT IF NOT EXISTS asignaturas_codigo_uk  UNIQUE (codigo);
> ```
