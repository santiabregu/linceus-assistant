...
def obtener_codigos_con_error(curso_academico: str) -> set:
    """
    Devuelve el conjunto de (codigo_asignatura, grupo) de planes con estado_rag = 'error'.
    Se usa con --solo-errores para filtrar qué PDFs re-procesar.
    """
    conn = db_client.get_connection()
    if not conn:
        return set()
